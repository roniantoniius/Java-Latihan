...
public class KapalDaoImlTests {
	...
	public void testThatFindGenerateKapalQuery() {
		underTest.find();
		verify(jdbcTemplate).query(
				eq("SELECT idkapal, namekapal, ukuran, ppnid FROM kapal"),
				ArgumentMatchers.<KapalDaoImpl.KapalRowMapper>any()
		);
	}
	
	@Test
	public void testThatUpdateGenerateKapalQuery() {
		Kapal kapal = TestDataUtil.createTestKapalA();
		underTest.update(kapal.getIdkapal(), kapal);
		verify(jdbcTemplate).update(
				"UPDATE kapal SET idkapal = ?, namekapal = ?, ukuran = ?, ppnid = ? WHERE idkapal = ?",
				1L, "Bintang", "3GT", 1L, 1L
		);
	}
	
	@Test
	public void testThatDeleteGenerateKapalQuery() {
		underTest.delete(1L);
		verify(jdbcTemplate).update(
				"DELETE FROM kapal WHERE idkapal = ?",
				1L
		);
	}
}
